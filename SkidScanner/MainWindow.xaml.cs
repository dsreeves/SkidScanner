...
namespace SkidScanner
{
	...
	{
		public string Skid { get; set; } = "Enter Stocktag #";
		public int Count { get; set; } = 0;
		public string LastScanned { get; set; } = "Scan Pump";
		...
		private void CheckCount()
		{
			...
			{
				SkidFinished fin = new SkidFinished();
				fin.ShowDialog();
				this.Show();
				Count = 0;
				_tagAcq = false;
				LastScanned = "New Skid. Scan Pump";
				skid.Text = "Enter Stocktag #";
				skid.Background = Brushes.White;
			}
		}
		...
		private void skid_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
		{
			string t = skid.Text;

			if (t.Length == 6 && t.Contains("-"))
			{
				_tagAcq = true;
				skid.Background = Brushes.White;
			}
			...
			{
				_tagAcq = false;
				skid.Background = Brushes.Yellow;
			}
			
			if (_tagAcq && (t.Contains("-") && t.Length >= 6))
			{
				Skid = t;
				_skid = new SkidLst(Skid);
				return;
			}
		}
